...
def entrainer_modeles(df_features):
    ...
    vprint("    Entrainement Baseline (moyenne)...")
    class MeanBaseline:
        def fit(self, X, y):
        ...
        def predict(self, X):
            X_arr = np.asarray(X)
            # Ensure 2D-like behavior: if 1D, treat length as samples
            if X_arr.ndim == 1:
                n = X_arr.shape[0]
            else:
                n = X_arr.shape[0]
    ...
    baseline = MeanBaseline().fit(X_train_robust, y_train_robust)
    baseline_train_pred = baseline.predict(X_train_final)
    baseline_val_pred = baseline.predict(X_test_robust)
    results['Baseline_mean'] = {
        'model': baseline,
        'scaler': None,
        'train_r2': r2_score(y_train_final, baseline_train_pred),
        'robust_r2': r2_score(y_test_robust, baseline_val_pred) if len(y_test_robust) > 0 else np.nan,
    ...
    # 2) LinearRegression
    vprint("    Entrainement LinearRegression...")
    lr = LinearRegression().fit(X_train_robust_scaled, y_train_robust)
    lr_robust_r2 = r2_score(y_test_robust, lr.predict(X_test_robust_scaled)) if len(y_test_robust) > 0 else np.nan
    # retrain final
    lr_final = LinearRegression().fit(X_train_final_scaled, y_train_final)
    results['LinearRegression'] = {
        'model': lr_final,
        'scaler': scaler_final,
        'train_r2': r2_score(y_train_final, lr_final.predict(X_train_final_scaled)),
        'robust_r2': lr_robust_r2,
        'y_train_pred': lr_final.predict(X_train_final_scaled),
        'y_test_pred': np.array([]),
        'feature_columns': feature_columns
    }
    ...
    # 5) RandomForest small grid
    vprint("    Recherche petite grille RandomForest (n_estimators)...")
    n_options = [20, 50, 100]
    best_rf = (None, None, -np.inf)
    for n in n_options:
        m = RandomForestRegressor(n_estimators=n, random_state=42)
        m.fit(X_train_robust, y_train_robust)  # RF handles unscaled
        if len(y_test_robust) > 0:
            val_r2 = r2_score(y_test_robust, m.predict(X_test_robust))
        else:
            val_r2 = -np.inf
        if val_r2 > best_rf[2]:
            best_rf = (m, n, val_r2)
    rf_final = RandomForestRegressor(n_estimators=best_rf[1], random_state=42).fit(X_train_final, y_train_final)
    results[f'RandomForest_n{best_rf[1]}'] = {
        'model': rf_final,
        'scaler': None,
        'train_r2': r2_score(y_train_final, rf_final.predict(X_train_final)),
        'robust_r2': best_rf[2],
